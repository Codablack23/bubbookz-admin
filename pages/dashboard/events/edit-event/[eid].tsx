...
function DropzoneWidget({action,title,img_url,}:DropZoneProps){
  const [file,setFile] = useState<FileList[]|{}>({})
  const [isUploading,setIsUploading] = useState(false)
  const [imageUrl,setImageUrl] = useState<any>(img_url?img_url:"")
     ...
     {(({getRootProps, getInputProps})=>
      <form action="" className="upload" onSubmit={handleUpload}>
          ...
          <div className="bg-gray-200 rounded-lg p-3 text-center">
            ...
            <Image src={imageUrl as string} height={200} alt="event-photo"/>
          </div>
          :
...
<div className="host-form">
    ...
    <DropzoneWidget action={handleUpload} img_url={host?.imageUrl as string} errors={""} title={host?.hostname}/>
...
function UploadBannerWidget({action,img_url}:WidgetProps){
  return(
    <div className="rounded bg-white p-3 bub-min-vh-40">
    <h3 className="font-bold">Add Cover Image</h3>
    <DropzoneWidget action={action} img_url={img_url as string} title={"Event"}/>
  </div>
  )
}
    ...
    async function handleSubmit(e:any){
      const hostErrors = hosts.length > 0 ? validateFields([
        ...
        if(allErrors.length == 0){
          if(dateDiff > 2){
           setLoading(true)
           const response = await Events.editEvent(router.query.eid as string,{
            name,
            contact,
            date:userDate,
            location,
     ...
     useEffect(()=>{
        const getEvent = async (id:string)=>{
            ...
            if(response.status === "success"){
              ...
              setLocationLink(e.location_link)
              setRegLink(e.registration_link)
              setDesc(e.description)
              setTime(/[0-9]+:[0-9]+(am)|(pm)/gi.exec(e.event_time)?
              {
                hour:parseInt(e.event_time.slice(0,e.event_time.indexOf(":"))),
                  minute:{
                    1:parseInt(e.event_time.slice(e.event_time.trim().indexOf(":")+1)[0]),
                    2:parseInt(e.event_time.slice(e.event_time.trim().indexOf(":") + 1)[1]),
                  },
                  meridian:e.event_time.slice(e.event_time.toLowerCase().indexOf("m") - 1)
              }
              :{hour:1,
                minute:{
                  1:0,
              ...
              setImageUrl(e.img_link)
              setHost(JSON.parse(e.hosts))    
              }
        }
        if(router.query.eid){
            getEvent(router.query.eid as string)
        }
     },[router])
    return(
    <DashboardLayout pageTitle={"Edit Event"} currentPage={"events"}>
      <>
      ...
      <div className="my-3 flex items-center">
        ...
        <Link href={"/dashboard/events"}><a>{name}</a></Link>
        <span className="block mx-1"> &gt;</span>
        <span className="font-bold">Edit Event</span></div>
      <div className="bub-grid mt-4 ">
        ...
        <div className="bub-min-vh-45 min-h-screen rounded-lg grid-col-5 grid-col-md-12">
          ...
          <UploadBannerWidget img_url={imageUrl} action={setImageUrl}/>
